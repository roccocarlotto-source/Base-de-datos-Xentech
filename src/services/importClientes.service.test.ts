...
test("parseWorkbookBuffer lee headers y filas, saltando filas totalmente vacias", async () => {
...
test("parseWorkbookBuffer rechaza un archivo que no es un Excel valido", async () => {
  await assert.rejects(() => parseWorkbookBuffer(Buffer.from("esto no es un xlsx")));
});
